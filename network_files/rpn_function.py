...
class AnchorsGenerator(nn.Module):
    ...
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        ...
        h_ratios = torch.sqrt(aspect_ratios)    # 每一个anchor的高度对应的乘法因子   为保证每一个anchor的面积相同形状不同
        w_ratios = 1.0 / h_ratios               # 每一个anchor的宽度对应的乘法因子

        # [r1, r2, r3]' * [s1, s2, s3]
        # number of elements is len(ratios)*len(scales)
        ws = (w_ratios[:, None] * scales[None, :]).view(-1)   # 每一个宽度比例乘每一个尺度（size）=每一个anchor的宽度
    ...
    def grid_anchors(self, grid_sizes, strides):
        ...
        # 遍历每个预测特征层的grid_size，strides和cell_anchors（anchor的中心点坐标）
    ...
    def forward(self, image_list, feature_maps):
        ...
        # 计算特征层上的一步等于原始图像上的步长   图像大小除特征层大小
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def assign_targets_to_anchors(self, anchors, targets):
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            if gt_boxes.numel() == 0:
                ...
                # 计算anchors与真实bbox的iou信息   match_quality_matrix就是iou的值
    ...
    def forward(self,
        ...
        # 调整内部tensor格式以及shape （从四维变成二维）———— objectness（8,15,25,38，）变成 （114000,1）
        ...
        # the proposals
        # 将预测的bbox regression参数应用到anchors上得到 （最终预测bbox坐标）
        ...
        if self.training:
            ...
            # 计算每个anchors最匹配的gt，并将anchors进行分类，前景，背景以及废弃的anchors  # 背景处的anchor就是matched_gt_boxes
